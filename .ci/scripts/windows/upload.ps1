...
if ("$BUILD_NAME" -eq "mainline") {
    $RELEASE_DIST = "torzu-windows-msvc"
}
else {
    $RELEASE_DIST = "torzu-windows-msvc-$BUILD_NAME"
}

$MSVC_BUILD_ZIP = "torzu-windows-msvc-$GITDATE-$GITREV.zip" -replace " ", ""
$MSVC_BUILD_PDB = "torzu-windows-msvc-$GITDATE-$GITREV-debugsymbols.zip" -replace " ", ""
$MSVC_SEVENZIP = "torzu-windows-msvc-$GITDATE-$GITREV.7z" -replace " ", ""
$MSVC_TAR = "torzu-windows-msvc-$GITDATE-$GITREV.tar" -replace " ", ""
$MSVC_TARXZ = "torzu-windows-msvc-$GITDATE-$GITREV.tar.xz" -replace " ", ""
$MSVC_SOURCE = "torzu-windows-msvc-source-$GITDATE-$GITREV" -replace " ", ""
...
if ("$env:GITHUB_ACTIONS" -eq "true") {
    ...
    cp .\build\bin\torzu*.exe .\artifacts\
    ...
    # Debugging symbols
    cp .\build\bin\torzu*.pdb .\artifacts\
...
rm "$RELEASE_DIST\*.exe"
Get-ChildItem "$BUILD_DIR" -Recurse -Filter "torzu*.exe" | Copy-Item -destination $RELEASE_DIST
