...
DiscordImpl::DiscordImpl(Core::System& system_) : system{system_} {
    ...
    // The number is the client ID for torzu, it's used for images and the
    // application name
...
void DiscordImpl::UpdateGameStatus(bool use_default) {
    const std::string default_text = "torzu is an emulator for the Nintendo Switch";
    const std::string default_image = "torzu_logo";
...
void DiscordImpl::Update() {
    ...
    if (system.IsPoweredOn()) {
        ...
        // Used to format Icon URL for torzu website game compatibility page
        std::string icon_name = GetGameString(game_title);
        game_url = fmt::format("https://torzu-emu.org/images/game/boxart/{}.png", icon_name);
