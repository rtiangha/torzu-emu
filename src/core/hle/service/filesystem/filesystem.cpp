...
std::shared_ptr<FileSys::SaveDataFactory> FileSystemController::CreateSaveDataFactory(
    ...
    using TorzuPath = Common::FS::TorzuPath;
    const auto rw_mode = FileSys::OpenMode::ReadWrite;

    auto vfs = system.GetFilesystem();
    const auto nand_directory =
        vfs->OpenDirectory(Common::FS::GetTorzuPathString(TorzuPath::NANDDir), rw_mode);
...
void FileSystemController::CreateFactories(FileSys::VfsFilesystem& vfs, bool overwrite) {
    ...
    using TorzuPath = Common::FS::TorzuPath;
    const auto sdmc_dir_path = Common::FS::GetTorzuPath(TorzuPath::SDMCDir);
    ...
    auto load_directory = vfs.OpenDirectory(Common::FS::GetTorzuPathString(TorzuPath::LoadDir),
                                            FileSys::OpenMode::Read);
    auto sd_load_directory = vfs.OpenDirectory(Common::FS::PathToUTF8String(sdmc_load_dir_path),
                                               FileSys::OpenMode::Read);
    auto dump_directory =
        vfs.OpenDirectory(Common::FS::GetTorzuPathString(TorzuPath::DumpDir), rw_mode);
