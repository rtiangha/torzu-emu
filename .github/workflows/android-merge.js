...
async function tagAndPushEA(github, owner, repo, execa) {
    ...
    console.log(`New tag: ${newTag}`);
    console.info('Pushing tags to GitHub ...');
    await execa("git", ["remote", "add", "android", "https://github.com/torzu-emu/torzu-android.git"]);
...
async function mergePullRequests(pulls, execa) {
    ...
    await execa("git", ["config", "--global", "user.name", "torzubot"]);
    await execa("git", [
        "config",
        "--global",
        "user.email",
        "torzu\x40torzu-emu\x2eorg", // prevent email harvesters from scraping the address
    ]);
    let hasFailed = false;
    for (let pull of pulls) {
        let pr = pull.number;
        ...
        try {
            ...
            const process2 = execa("git", ["commit", "-m", `Merge torzu-emu#${pr}`]);
...
async function resetBranch(execa) {
    ...
    let hasFailed = false;
    try {
        await execa("git", ["remote", "add", "source", "https://github.com/torzu-emu/torzu.git"]);
...
async function getMainlineTag(execa) {
    ...
    let hasFailed = false;
    try {
        await execa("git", ["remote", "add", "mainline", "https://github.com/torzu-emu/torzu-android.git"]);
...
async function mergebot(github, context, execa) {
    // Reset our local copy of master to what appears on torzu-emu/torzu - master
    ...
    await fetchPullRequests(pulls, "https://github.com/torzu-emu/torzu", execa);
    const mergeResults = await mergePullRequests(pulls, execa);

    if (BUILD_EA) {
        await tagAndPushEA(github, 'torzu-emu', `torzu-android`, execa);
    } else {
        await generateReadme(pulls, context, mergeResults, execa);
        await tagAndPush(github, 'torzu-emu', `torzu-android`, execa, true);
    }
}
