...
You'll need to download and install the following to build torzu:
...
- Ubuntu / Linux Mint / Debian:
  ...
  -  Users need to manually specify building with QT Web Engine enabled.  This is done using the parameter `-DTORZU_USE_QT_WEB_ENGINE=ON` when running CMake. 
  ...
  - Users need to manually disable building SDL2 from externals if they intend to use the version provided by their system by adding the parameters `-DTORZU_USE_EXTERNAL_SDL2=OFF`
...
- Fedora:
  ...
  - CMake arguments to force system libraries:
    - SDL2: `-DTORZU_USE_BUNDLED_SDL2=OFF -DTORZU_USE_EXTERNAL_SDL2=OFF`
    - FFmpeg: `-DTORZU_USE_EXTERNAL_FFMPEG=OFF`
...
### Building torzu in Release Mode (Optimized)

If you need to run ctests, you can disable `-DTORZU_TESTS=OFF` and install Catch2.

```bash
mkdir build && cd build
cmake .. -GNinja -DTORZU_USE_BUNDLED_VCPKG=ON -DTORZU_TESTS=OFF
ninja
sudo ninja install 
```
...
### Building torzu in Debug Mode (Slow)

```bash
mkdir build && cd build
cmake .. -GNinja -DCMAKE_BUILD_TYPE=Debug -DTORZU_USE_BUNDLED_VCPKG=ON -DTORZU_TESTS=OFF
ninja
```
...
```bash
mkdir build && cd build
cmake .. -GNinja -DCMAKE_BUILD_TYPE=RelWithDebInfo -DTORZU_USE_BUNDLED_VCPKG=ON -DTORZU_TESTS=OFF
ninja
```

### Running without installing

After building, the binaries `torzu` and `torzu-cmd` (depending on your build options) will end up in `build/bin/`.
...
1. Enable CPU debugging
   ![](https://raw.githubusercontent.com/flathub/org.torzu_emu.torzu/master/assets/torzu-settings-2.png)
2. Disable both Host MMU emulation options
   ![](https://raw.githubusercontent.com/flathub/org.torzu_emu.torzu/master/assets/torzu-settings-1.png)
3. Run gdb

```bash
cd data
gdb ../build/bin/torzu            # Start GDB
(gdb) handle SIGSEGV nostop      # Disable SIGSEGV exceptions, which are used by torzu for memory access
(gdb) run                        # Run torzu under GDB
<crash>
