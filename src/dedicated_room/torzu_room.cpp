...
static void PrintHelp(const char* argv0) {
    LOG_INFO(Network,
             ...
             "--web-api-url       torzu Web API url\n"
             "--ban-list-file     The file for storing the room ban list\n"
             "--log-file          The file for storing the room log\n"
             "--enable-torzu-mods Allow torzu Community Moderators to moderate on your room\n"
...
static void PrintVersion() {
    LOG_INFO(Network, "torzu dedicated room {} {} Libnetwork: {}", Common::g_scm_branch,
             Common::g_scm_desc, Network::network_version);
}

/// The magic text at the beginning of a torzu-room ban list file.
static constexpr char BanListMagic[] = "TorzuRoom-BanList-1";
...
int main(int argc, char** argv) {
    ...
    std::string log_file = "torzu-room.log";
    std::string bind_address;
    u64 preferred_game_id = 0;
    u32 port = Network::DefaultRoomPort;
    u32 max_members = 16;
    bool enable_torzu_mods = false;
    ...
    static struct option long_options[] = {
        ...
        {"enable-torzu-mods", no_argument, 0, 'e'},
    ...
    if (announce) {
        if (username.empty()) {
            ...
            PadToken(token);
            Settings::values.torzu_username = UsernameFromDisplayToken(token);
            username = Settings::values.torzu_username.GetValue();
            Settings::values.torzu_token = TokenFromDisplayToken(token);
        } else {
            LOG_INFO(Network, "Hosting a public room");
            Settings::values.web_api_url = web_api_url;
            Settings::values.torzu_username = username;
            Settings::values.torzu_token = token;
        }
    }
    if (!announce && enable_torzu_mods) {
        enable_torzu_mods = false;
        LOG_INFO(Network, "Can not enable torzu Moderators for private rooms");
    }
...
#else
        LOG_INFO(Network,
                 "torzu Web Services is not available with this build: validation is disabled.");
    ...
    if (auto room = network.GetRoom().lock()) {
        ...
        if (!room->Create(room_name, room_description, bind_address, static_cast<u16>(port),
                          ...
                          std::move(verify_backend), ban_list, enable_torzu_mods)) {
