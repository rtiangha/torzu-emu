...
#include "torzu/multiplayer/client_room.h"
#include "torzu/multiplayer/lobby.h"
#include "torzu/multiplayer/lobby_p.h"
...
Lobby::Lobby(QWidget* parent, QStandardItemModel* list,
    ...
    if (ui->nickname->text().isEmpty() || ui->nickname->text() == QStringLiteral("torzu")) {
        if (!Settings::values.torzu_username.GetValue().empty()) {
            ui->nickname->setText(
                QString::fromStdString(Settings::values.torzu_username.GetValue()));
        } else if (!GetProfileUsername().empty()) {
            ui->nickname->setText(QString::fromStdString(GetProfileUsername()));
        } else {
            ui->nickname->setText(QStringLiteral("torzu"));
        }
    }
...
#ifdef ENABLE_WEB_SERVICE
        if (!Settings::values.torzu_username.GetValue().empty() &&
            !Settings::values.torzu_token.GetValue().empty()) {
