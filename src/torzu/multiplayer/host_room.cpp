...
#include "torzu/multiplayer/host_room.h"
#include "torzu/multiplayer/message.h"
#include "torzu/multiplayer/state.h"
#include "torzu/multiplayer/validation.h"
...
HostRoomWindow::HostRoomWindow(QWidget* parent, QStandardItemModel* list,
    ...
    if (ui->username->text().isEmpty() && !Settings::values.torzu_username.GetValue().empty()) {
        // Use torzu Web Service user name as nickname by default
        ui->username->setText(QString::fromStdString(Settings::values.torzu_username.GetValue()));
    }
    ...
    if (auto member = room_network.GetRoomMember().lock()) {
        ...
        if (auto room = room_network.GetRoom().lock()) {
            const bool created =
                room->Create(ui->room_name->text().toStdString(),
                             ...
                             ui->max_player->value(), Settings::values.torzu_username.GetValue(),
                ...
                if (result.result_code != WebService::WebResult::Code::Success) {
                    QMessageBox::warning(
                        ...
                        tr("Failed to announce the room to the public lobby. In order to host a "
                           "room publicly, you must have a valid torzu account configured in "
        ...
        if (is_public) {
            WebService::Client client(Settings::values.web_api_url.GetValue(),
                                      Settings::values.torzu_username.GetValue(),
                                      Settings::values.torzu_token.GetValue());
            if (auto room = room_network.GetRoom().lock()) {
