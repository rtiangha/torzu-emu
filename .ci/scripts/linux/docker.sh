...
rm -vf AppDir/usr/bin/torzu-cmd AppDir/usr/bin/torzu-tester

# Download tools needed to build an AppImage
wget -nc https://raw.githubusercontent.com/torzu-emu/ext-linux-bin/main/appimage/deploy-linux.sh
wget -nc https://raw.githubusercontent.com/torzu-emu/AppImageKit-checkrt/old/AppRun.sh
wget -nc https://github.com/torzu-emu/ext-linux-bin/raw/main/appimage/exec-x86_64.so
# Set executable bit
chmod 755 \
    deploy-linux.sh \
    AppRun.sh \
...
# Deploy torzu's needed dependencies
DEPLOY_QT=1 ./deploy-linux.sh AppDir/usr/bin/torzu AppDir
...
# Workaround for building torzu with GCC 10 but also trying to distribute it to Ubuntu 18.04 et al.
