...
On Windows, all library dependencies are automatically included within the `externals` folder, or can be downloaded on-demand. To build torzu, you need to install:
...
* *(Note: torzu by default downloads to `C:\Users\<user-name>\torzu` (Master) or `C:\Users\<user-name>\torzu-mainline` (Mainline)*
...
* For the build directory, use a `/build` subdirectory inside the source directory or some other directory of your choice. (Tell CMake to create it.)
  ...
  ![11](https://github.com/torzu-emu/torzu/assets/20753089/738efcab-0da6-44ce-889d-becf3712db10)
...
* If you get an error about missing packages, enable `TORZU_USE_BUNDLED_VCPKG`, and then click Configure again.

  * *(You may also want to disable `TORZU_TESTS` in this case since Catch2 is not yet supported with this.)*
...
* Open the solution file `torzu.sln` in Visual Studio 2022, which is located in the build folder.

  ![16](https://i.imgur.com/208yMml.png)

* Depending if you want a graphical user interface or not (`torzu` has the graphical user interface, while `torzu-cmd` doesn't), select `torzu` or `torzu-cmd` in the Solution Explorer, right-click and `Set as StartUp Project`.
...
### Install torzu dependencies for MinGW-w64
...
### Run the following commands to build torzu (dynamically linked build)

```bash
mkdir build && cd build
cmake -G "MSYS Makefiles" -DTORZU_USE_BUNDLED_VCPKG=ON -DTORZU_TESTS=OFF ..
make -j$(nproc)
# test torzu out with
./bin/torzu.exe
```
...
To build torzu, you need to install the following:
...
* Once this process has been completed (No loading bar bottom right), you can now build torzu
* In the top right, click on the drop-down menu, select all configurations, then select torzu

![6](https://user-images.githubusercontent.com/42481638/216899226-975048e9-bc6d-4ec1-bc2d-bd8a1e15ed04.png)

* Now run by clicking the play button or pressing Shift+F10, and torzu will auto-launch once built.
