...
APPIMAGE_NAME="torzu-${RELEASE_NAME}-${GITDATE}-${GITREV}.AppImage"
BASE_NAME="torzu-linux"
...
cp build/bin/torzu-cmd "$DIR_NAME"
if [ "${RELEASE_NAME}" != "early-access" ] && [ "${RELEASE_NAME}" != "mainline" ]; then
    cp build/bin/torzu "$DIR_NAME"
fi

# Build an AppImage
cd build

wget -nc https://github.com/torzu-emu/ext-linux-bin/raw/main/appimage/appimagetool-x86_64.AppImage
...
if [ "${RELEASE_NAME}" = "mainline" ] || [ "${RELEASE_NAME}" = "early-access" ]; then
    echo "X-AppImage-Integrate=false" >> AppDir/org.torzu_emu.torzu.desktop
fi

if [ "${RELEASE_NAME}" = "mainline" ]; then
    # Generate update information if releasing to mainline
    ./appimagetool-x86_64.AppImage -u "gh-releases-zsync|torzu-emu|torzu-${RELEASE_NAME}|latest|torzu-*.AppImage.zsync" AppDir "${APPIMAGE_NAME}"
else
...
if [ "${RELEASE_NAME}" = "mainline" ] || [ "${RELEASE_NAME}" = "early-access" ]; then
    cp "build/${APPIMAGE_NAME}" "${DIR_NAME}/torzu-${RELEASE_NAME}.AppImage"
fi
