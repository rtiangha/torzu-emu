...
enum class TorzuPath {
    TorzuDir,        // Where torzu stores its data.
    ...
    ScreenshotsDir, // Where torzu screenshots are stored.
...
/**
 * Gets the filesystem path associated with the TorzuPath enum.
 *
 * @param torzu_path TorzuPath enum
 *
 * @returns The filesystem path associated with the TorzuPath enum.
 */
[[nodiscard]] const std::filesystem::path& GetTorzuPath(TorzuPath torzu_path);

/**
 * Gets the filesystem path associated with the TorzuPath enum as a UTF-8 encoded std::string.
 *
 * @param torzu_path TorzuPath enum
 *
 * @returns The filesystem path associated with the TorzuPath enum as a UTF-8 encoded std::string.
 */
[[nodiscard]] std::string GetTorzuPathString(TorzuPath torzu_path);

/**
 * Sets a new filesystem path associated with the TorzuPath enum.
 * If the filesystem object at new_path is not a directory, this function will not do anything.
 *
 * @param torzu_path TorzuPath enum
 * @param new_path New filesystem path
 */
void SetTorzuPath(TorzuPath torzu_path, const std::filesystem::path& new_path);

#ifdef _WIN32
template <typename Path>
void SetTorzuPath(TorzuPath torzu_path, const Path& new_path) {
    if constexpr (IsChar<typename Path::value_type>) {
        SetTorzuPath(torzu_path, ToU8String(new_path));
    } else {
        SetTorzuPath(torzu_path, std::filesystem::path{new_path});
    }
}
#endif
...
/**
 * Gets the relevant paths for torzu to store its data based on the given XDG environment variable.
 ...
 *
 * @returns The path where torzu should store its data.
 */
...
// i.e. "C:\Users\Torzu\Documents\save.bin" becomes {"C:", "Users", "Torzu", "Documents", "save.bin" }
