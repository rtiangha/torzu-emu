...
#include "torzu/multiplayer/state.h"
#include "torzu/util/controller_navigation.h"
...
#include "torzu/bootmanager.h"
#include "torzu/compatdb.h"
#include "torzu/compatibility_list.h"
#include "torzu/configuration/configure_dialog.h"
#include "torzu/configuration/configure_input_per_game.h"
#include "torzu/configuration/qt_config.h"
#include "torzu/debugger/console.h"
#include "torzu/debugger/controller.h"
#include "torzu/debugger/profiler.h"
#include "torzu/debugger/wait_tree.h"
#include "torzu/discord.h"
#include "torzu/game_list.h"
#include "torzu/game_list_p.h"
#include "torzu/hotkeys.h"
#include "torzu/install_dialog.h"
#include "torzu/loading_screen.h"
#include "torzu/main.h"
#include "torzu/play_time_manager.h"
#include "torzu/startup_checks.h"
#include "torzu/uisettings.h"
#include "torzu/util/clickable_label.h"
#include "torzu/vk_device_info.h"

#ifdef TORZU_CRASH_DUMPS
#include "torzu/breakpad.h"
#endif

using namespace Common::Literals;

#ifdef USE_DISCORD_PRESENCE
#include "torzu/discord_impl.h"
#endif
...
static void RemoveCachedContents() {
    const auto cache_dir = Common::FS::GetTorzuPath(Common::FS::TorzuPath::CacheDir);
...
GMainWindow::GMainWindow(std::unique_ptr<QtConfig> config_, bool has_broken_vulkan)
    ...
    LOG_INFO(Frontend, "torzu Version: {}", torzu_build_version);
    LogRuntimes();
#ifdef ARCHITECTURE_x86_64
    ...
    if (has_broken_vulkan) {
        ...
        QMessageBox::warning(this, tr("Broken Vulkan Installation Detected"),
                             tr("Vulkan initialization failed during boot.<br><br>Click <a "
                                "href='https://torzu-emu.org/wiki/faq/"
                                "#torzu-starts-with-the-error-broken-vulkan-installation-detected'>"
    ...
    //: TRANSLATORS: This string is shown to the user to explain why torzu needs to prevent the
...
#ifdef TORZU_ENABLE_COMPATIBILITY_REPORTING
...
void GMainWindow::InitializeHotkeys() {
    ...
    LinkActionShortcut(ui->action_Exit, QStringLiteral("Exit torzu"));
...
void GMainWindow::ConnectMenuEvents() {
    ...
    // Help
    connect_menu(ui->action_Open_torzu_Folder, &GMainWindow::OnOpenTorzuFolder);
    ...
    if (result == Core::SystemResultStatus::Success &&
        ...
        QMessageBox::warning(
            ...
            tr("You are using the deconstructed ROM directory format for this game, which is an "
               ...
               "support.<br><br>For an explanation of the various Switch formats torzu supports, <a "
               "href='https://torzu-emu.org/wiki/overview-of-switch-game-formats'>check out our "
    ...
    if (result != Core::SystemResultStatus::Success) {
        ...
        case Core::SystemResultStatus::ErrorVideoCore:
            QMessageBox::critical(
                ...
                tr("torzu has encountered an error while running the video core. "
                   "This is usually caused by outdated GPU drivers, including integrated ones. "
                   "Please see the log for more details. "
                   "For more information on accessing the log, please see the following page: "
                   "<a href='https://torzu-emu.org/help/reference/log-files/'>"
        ...
        default:
            if (result > Core::SystemResultStatus::ErrorLoader) {
                ...
                const auto description =
                    tr("%1<br>Please follow <a href='https://torzu-emu.org/help/quickstart/'>the "
                       "torzu quickstart guide</a> to redump your files.<br>You can refer "
                       "to the torzu wiki</a> or the torzu Discord</a> for help.",
...
void GMainWindow::BootGame(const QString& filename, Service::AM::FrontendAppletParameters params,
                           StartGameType type) {
    LOG_INFO(Frontend, "torzu starting...");
...
void GMainWindow::OnGameListOpenFolder(u64 program_id, GameListOpenTarget target,
    ...
    case GameListOpenTarget::SaveData: {
        ...
        const auto nand_dir = Common::FS::GetTorzuPath(Common::FS::TorzuPath::NANDDir);
        auto vfs_nand_dir =
    ...
    case GameListOpenTarget::ModData: {
        ...
        path = Common::FS::GetTorzuPath(Common::FS::TorzuPath::LoadDir) /
...
void GMainWindow::OnTransferableShaderCacheOpenFile(u64 program_id) {
    const auto shader_cache_dir = Common::FS::GetTorzuPath(Common::FS::TorzuPath::ShaderDir);
...
void GMainWindow::OnGameListRemoveInstalledEntry(u64 program_id, InstalledEntryType type) {
    ...
    Common::FS::RemoveDirRecursively(Common::FS::GetTorzuPath(Common::FS::TorzuPath::CacheDir) /
                                     "game_list");
    game_list->PopulateAsync(UISettings::values.game_dirs);
}
...
void GMainWindow::RemoveCustomConfiguration(u64 program_id, const std::string& game_path) {
    ...
    const auto custom_config_file_path =
        Common::FS::GetTorzuPath(Common::FS::TorzuPath::ConfigDir) / "custom" / config_file_name;
...
void GMainWindow::OnGameListDumpRomFS(u64 program_id, const std::string& game_path,
    ...
    const auto dump_dir =
        target == DumpRomFSTarget::Normal
            ? Common::FS::GetTorzuPath(Common::FS::TorzuPath::DumpDir)
            : Common::FS::GetTorzuPath(Common::FS::TorzuPath::SDMCDir) / "atmosphere" / "contents";
...
void GMainWindow::OnGameListNavigateToGamedbEntry(u64 program_id,
    ...
    QDesktopServices::openUrl(QUrl(QStringLiteral("https://torzu-emu.org/game/") + directory));
}
...
bool GMainWindow::MakeShortcutIcoPath(const u64 program_id, const std::string_view game_file_name,
    ...
    // Get path to Torzu icons directory & icon extension
    std::string ico_extension = "png";
#if defined(_WIN32)
    out_icon_path = Common::FS::GetTorzuPath(Common::FS::TorzuPath::IconsDir);
    ico_extension = "ico";
    ...
    out_icon_path /= (program_id == 0 ? fmt::format("torzu-{}.{}", game_file_name, ico_extension)
                                      : fmt::format("torzu-{:016X}.{}", program_id, ico_extension));
    return true;
}

void GMainWindow::OnGameListCreateShortcut(u64 program_id, const std::string& game_path,
                                           GameListShortcutTarget target) {
    // Get path to torzu executable
    const QStringList args = QApplication::arguments();
    std::filesystem::path torzu_command = args[0].toStdString();
    // If relative path, make it an absolute path
    if (torzu_command.c_str()[0] == '.') {
        torzu_command = Common::FS::GetCurrentDir() / torzu_command;
    }
    // Shortcut path
    ...
    if (torzu_command.string().ends_with(appimage_ending) &&
    ...
    const std::string comment = fmt::format("Start {:s} with the torzu Emulator", game_title);
    const std::string categories = "Game;Emulator;Qt;";
    const std::string keywords = "Switch;Nintendo;";

    if (GMainWindow::CreateShortcutLink(shortcut_path, comment, out_icon_path, torzu_command,
...
void GMainWindow::OnGameListOpenDirectory(const QString& directory) {
    ...
    if (directory == QStringLiteral("SDMC")) {
        fs_path =
            Common::FS::GetTorzuPath(Common::FS::TorzuPath::SDMCDir) / "Nintendo/Contents/registered";
    } else if (directory == QStringLiteral("UserNAND")) {
        fs_path =
            Common::FS::GetTorzuPath(Common::FS::TorzuPath::NANDDir) / "user/Contents/registered";
    } else if (directory == QStringLiteral("SysNAND")) {
        fs_path =
            Common::FS::GetTorzuPath(Common::FS::TorzuPath::NANDDir) / "system/Contents/registered";
    } else {
...
void GMainWindow::OnConfigure() {
    ...
    if (result != QDialog::Accepted && !UISettings::values.configuration_applied &&
        ...
        if (!Common::FS::RemoveDirContentsRecursively(
                Common::FS::GetTorzuPath(Common::FS::TorzuPath::ConfigDir) / "custom")) {
            LOG_WARNING(Frontend, "Failed to remove custom configuration files");
        }
        if (!Common::FS::RemoveDirRecursively(
                Common::FS::GetTorzuPath(Common::FS::TorzuPath::CacheDir) / "game_list")) {
...
void GMainWindow::OnOpenTorzuFolder() {
    QDesktopServices::openUrl(QUrl::fromLocalFile(
        QString::fromStdString(Common::FS::GetTorzuPathString(Common::FS::TorzuPath::TorzuDir))));
}
...
void GMainWindow::OnInstallFirmware() {
    ...
    if (!ContentManager::AreKeysPresent()) {
        QMessageBox::information(
            ...
            tr("Install decryption keys and restart torzu before attempting to install firmware."));
        return;
    }
        ...
        if (QtProgressCallback(
            ...
            QMessageBox::warning(
                ...
                tr("Firmware installation cancelled, firmware may be in bad state, "
                   "restart torzu or re-install firmware."));
            return;
        }
    }
...
void GMainWindow::OnInstallDecryptionKeys() {
    ...
    const auto torzu_keys_dir = Common::FS::GetTorzuPath(Common::FS::TorzuPath::KeysDir);
    for (auto key_file : source_key_files) {
        std::filesystem::path destination_key_file = torzu_keys_dir / key_file.filename();
...
void GMainWindow::OnCaptureScreenshot() {
    ...
    const auto screenshot_path =
        QString::fromStdString(Common::FS::GetTorzuPathString(Common::FS::TorzuPath::ScreenshotsDir));
    const auto date =
...
void GMainWindow::MigrateConfigFiles() {
    const auto config_dir_fs_path = Common::FS::GetTorzuPath(Common::FS::TorzuPath::ConfigDir);
...
void GMainWindow::UpdateWindowTitle(std::string_view title_name, std::string_view title_version,
    ...
    const auto torzu_title = fmt::format("torzu | {}-{}", branch_name, description);
    const auto override_title =
        fmt::format(fmt::runtime(std::string(Common::g_title_bar_format_idle)), build_id);
    const auto window_title = override_title.empty() ? torzu_title : override_title;
    ...
    if (!ContentManager::AreKeysPresent()) {
        QMessageBox::warning(
            ...
            tr("Encryption keys are missing. "
               "<br>Please follow <a href='https://torzu-emu.org/help/quickstart/'>the torzu "
...
bool GMainWindow::ConfirmClose() {
    ...
    const auto text = tr("Are you sure you want to close torzu?");
    return question(this, tr("torzu"), text);
}
...
bool GMainWindow::ConfirmForceLockedExit() {
    ...
    const auto text = tr("The currently running application has requested torzu to not exit.\n\n"
...
int main(int argc, char* argv[]) {
    ...
    QCoreApplication::setOrganizationName(QStringLiteral("torzu team"));
    QCoreApplication::setApplicationName(QStringLiteral("torzu"));
