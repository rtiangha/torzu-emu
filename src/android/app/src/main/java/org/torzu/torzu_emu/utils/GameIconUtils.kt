...
object GameIconUtils {
    private val imageLoader = ImageLoader.Builder(TorzuApplication.appContext)
        .components {
            add(GameIconKeyer())
            add(GameIconFetcher.Factory())
        }
        .memoryCache {
            MemoryCache.Builder(TorzuApplication.appContext)
                .maxSizePercent(0.25)
                .build()
        }
        .build()
    ...
    fun loadGameIcon(game: Game, imageView: ImageView) {
        val request = ImageRequest.Builder(TorzuApplication.appContext)
            .data(game)
            .target(imageView)
    ...
    suspend fun getShortcutIcon(lifecycleOwner: LifecycleOwner, game: Game): IconCompat {
        val layerDrawable = ResourcesCompat.getDrawable(
            TorzuApplication.appContext.resources,
            R.drawable.shortcut,
            null
        ) as LayerDrawable
        layerDrawable.setDrawableByLayerId(
            R.id.shortcut_foreground,
            getGameIcon(lifecycleOwner, game).toDrawable(TorzuApplication.appContext.resources)
        )
        val inset = TorzuApplication.appContext.resources
