...
void ConfigureWeb::SetConfiguration() {
    ...
    ui->edit_token->setText(QString::fromStdString(Settings::values.torzu_token.GetValue()));
...
void ConfigureWeb::ApplyConfiguration() {
    ...
    if (Settings::values.torzu_username.GetValue().empty()) {
        ...
        Settings::values.torzu_username = "torzu";
    } else {
        // If a name already exist, reassign it to itself (needed for change set with a profile switch)
        Settings::values.torzu_username = Settings::values.torzu_username.GetValue();
    }

    if (ui->edit_token->text().isEmpty()) {
        // If no token specified, automatically generate one
        Settings::values.torzu_token = Common::UUID::MakeRandom().FormattedString();
    } else {
        // Otherwise use user-specified value
        Settings::values.torzu_token = ui->edit_token->text().toStdString();
    }
}
