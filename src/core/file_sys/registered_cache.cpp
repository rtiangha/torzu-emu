...
std::optional<NcaID> RegisteredCache::GetNcaIDFromMetadata(u64 title_id,
    ...
    const auto res1 = CheckMapForContentRecord(torzu_meta, title_id, type);
    if (res1)
        return res1;
...
void RegisteredCache::AccumulateTorzuMeta() {
    const auto meta_dir = dir->GetSubdirectory("torzu_meta");
    if (meta_dir == nullptr) {
        return;
    }
    ...
    for (const auto& file : meta_dir->GetFiles()) {
        ...
        CNMT cnmt(file);
        torzu_meta.insert_or_assign(cnmt.GetTitleID(), std::move(cnmt));
    }
}
...
std::optional<u32> RegisteredCache::GetEntryVersion(u64 title_id) const {
    ...
    const auto torzu_meta_iter = torzu_meta.find(title_id);
    if (torzu_meta_iter != torzu_meta.cend()) {
        return torzu_meta_iter->second.GetTitleVersion();
    }
...
InstallResult RegisteredCache::InstallEntry(const NCA& nca, TitleType type,
    ...
    if (!RawInstallTorzuMeta(new_cnmt)) {
        return InstallResult::ErrorMetaFailed;
    }
...
bool RegisteredCache::RemoveExistingEntry(u64 title_id) const {
    ...
    // If patch entries for any program exist in torzu meta, remove them
    for (u8 i = 0; i < 0x10; i++) {
        const auto meta_dir = dir->CreateDirectoryRelative("torzu_meta");
...
bool RegisteredCache::RawInstallTorzuMeta(const CNMT& cnmt) {
    ...
    Refresh();
    return std::find_if(torzu_meta.begin(), torzu_meta.end(),
