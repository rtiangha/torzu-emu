...
docker run -e AZURECIREPO -e TITLEBARFORMATIDLE -e TITLEBARFORMATRUNNING -e DISPLAYVERSION -e ENABLE_COMPATIBILITY_REPORTING -e CCACHE_DIR=/torzu/ccache -v "$(pwd):/torzu" -w /torzu torzuemu/build-environments:linux-fresh /bin/bash /torzu/.ci/scripts/linux/docker.sh "$1"
sudo chown -R $UID ./
