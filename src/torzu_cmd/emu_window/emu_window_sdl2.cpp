...
#include "torzu_cmd/emu_window/emu_window_sdl2.h"
#include "torzu_cmd/torzu_icon.h"
...
void EmuWindow_SDL2::WaitEvent() {
    ...
    if (current_time > last_time + 2000) {
        ...
        const auto title =
            fmt::format("torzu {} | {}-{} | FPS: {:.0f} ({:.0f}%)", Common::g_build_fullname,
...
void EmuWindow_SDL2::SetWindowIcon() {
    SDL_RWops* const torzu_icon_stream = SDL_RWFromConstMem((void*)torzu_icon, torzu_icon_size);
    if (torzu_icon_stream == nullptr) {
        LOG_WARNING(Frontend, "Failed to create torzu icon stream.");
        return;
    }
    SDL_Surface* const window_icon = SDL_LoadBMP_RW(torzu_icon_stream, 1);
