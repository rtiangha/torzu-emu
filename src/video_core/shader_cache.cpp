...
void ShaderCache::Register(std::unique_ptr<ShaderInfo> data, VAddr addr, size_t size) {
    ...
    const u64 page_end = (addr_end + TORZU_PAGESIZE - 1) >> TORZU_PAGEBITS;
    for (u64 page = addr >> TORZU_PAGEBITS; page < page_end; ++page) {
...
void ShaderCache::RemoveEntryFromInvalidationCache(const Entry* entry) {
    const u64 page_end = (entry->addr_end + TORZU_PAGESIZE - 1) >> TORZU_PAGEBITS;
    for (u64 page = entry->addr_start >> TORZU_PAGEBITS; page < page_end; ++page) {
